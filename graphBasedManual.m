function stats = graphBasedManual(imMedian, waterShedSegmentation, k, imageFixed, xMin, yMin, height_, width_, location, spineId, index_, angle, binDendPiece, xPixelSize)
...
if (sum(double(waterShedSegmentation(:)))/255) > ((size(waterShedSegmentation,1)*size(waterShedSegmentation,2))/2)
    ...
    waterShedSegmentation = ((((lb > 0)) > 0) & B)*255;
end
